...
module.exports = class CladeController extends Controller {
  constructor() {
    super(AccessControl);
  }
  ...
  getClades(req, res, next) {
    ...
    Clade.findById(nodeId, "-__v", (err, clade) => {
      if (clade) {
        ...
        this.handleResponse(res, next, err, {
          root: {},
          depth,
          total: CladeController.total
        });
      }
    });
  }
  ...
  searchForClades(req, res, next) {
    ...
    Clade.find(
      { name: { $regex: `^${name}`, $options: "i" }, _id: { $ne: self } },
      "name",
