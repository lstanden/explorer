...
const { SES } = require("@explorer/common/aws");
const { config } = require("@explorer/common/config");
...
const AuthenticationController = {
  ...
  _validatePasswordResetForm: payload => {
    ...
    if (
      !payload ||
      typeof payload.repeat_password !== "string" ||
      payload.repeat_password.trim().length === 0
    ) {
      isFormValid = false;
    ...
    if (isFormValid && payload.repeat_password !== payload.password) {
      ...
      errors.repeat_password =
        "The second password does not match the first one.";
    }
  ...
  signup: (req, res, next) => {
    const validationResult = AuthenticationController._validateSignupForm(
      req.body
    );
  ...
  login: (req, res, next) => {
    const validationResult = AuthenticationController._validateLoginForm(
      req.body
    );
    if (!validationResult.success) {
      return res.status(400).json({
  ...
  forgot: (req, res, next) => {
    const validationResult = AuthenticationController._validateForgotForm(
      req.body
    );
      ...
      (err, user) => {
        ...
        if (!user) {
          return res.status(400).json({
            success: false,
            message:
              "Can't find username or email. Check the spelling. This user may not exist -- you should register."
          });
        }
        ...
        user.save((err, user) => {
          if (err) {
            console.error(
              "User update error: Can't save password reset code.",
              err
            );
            return res.status(400).json({
              success: false,
          ...
          SES.sendEmailTemplate(user.email, "PasswordResetCode", {
            ...
            url: `https://${config.url}/password-reset?resetCode=${
  ...
  passwordReset: (req, res, next) => {
    const validationResult = AuthenticationController._validatePasswordResetForm(
      req.body
    );
    if (!validationResult.success) {
      return res.status(400).json({
      ...
      (err, user) => {
        ...
        if (!user) {
          return res.status(400).json({
            success: false,
            message:
              "Invalid reset code. This one might be expired. Reset your password again."
          });
        }
