...
module.exports = connection.model("Clade", CladeSchema);

function scoreResult(query, result) {
  let nameDistance = Levenshtein.get(query, result.name, { useCollator: true });
  let otherNamesDistance = result.otherNames
    ? reduceOtherNamesToSmallestDistance(query, result.otherNames)
    : 9999;
  ...
  return otherNames
    ...
    .reduce(
      (previous, current) =>
        Math.min(
          previous,
          Levenshtein.get(query, current, { useCollator: true })
        ),
      9999
    );
}
